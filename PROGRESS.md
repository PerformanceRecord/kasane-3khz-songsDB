...
- 2026-09-03: 実運用の統合GASを `google-apps-script-reference/merge-songs-gags-archive.gs` の保存用正本とし、Apps Scriptへの反映は手動で行う運用を明文化。
- 2026-09-03: 楽曲整理をシート単位の処理へ変更。メインは完全URL重複削除 → 同区分・同日再アップロード置換 → 区分優先度で1件化、アーカイブは完全重複と再アップロードだけを整理し、固有行と既存の所属・並び順を維持する方針を確定。
- 2026-09-03: 日常用仕分けの対象を新規行と同一楽曲だけから全件重複検査へ変更。チェックポイントは新規URLの優先判定だけに使用し、新規0件・初回でも日常用単独で既存重複を整理する。総点検の処理内容は変更せず、別途要件確定後に扱う。
- 2026-09-03: GASメニューを日常仕分け・完全重複総点検・近似情報チェック・統計の4機能へ整理し、動画監査3機能と統合一覧再構築を撤去。総点検はA+B+完全URL重複をアーカイブ側から優先削除し、近似情報チェックはテレコ・区切り記号差・両項目70%以上・完全URL一致を別シートへ指摘する方針を確定。
- 2026-09-03: 手動Action「マニュアルでアップロードデータの整理を行う」を追加。年数回の利用を想定し、全シートをページングで新規取得して件数・履歴参照を検証し、R2更新前バックアップ後に `songs/gags/meta/history` を完全同期する。通常のローリング同期は維持する。
...
- 手動全件整理中にスプレッドシートの行追加・削除・並び替えが発生するとページ境界が変わるリスク（件数変化は自動検知し、運用上も実行中の編集を避ける）。
