(function(){
  ...
  var state={category:'ショート',songs:[],gags:[],visible:[],playerVisible:false,playing:null};
  ...
  function render(){
    ...
    state.visible.forEach(function(row,index){
      ...
      actions.appendChild(createButton('履歴','history',function(event){event.stopPropagation();openHistory(row);}));
      if(row.dUrl){
        var link=document.createElement('a'); link.href=row.dUrl; link.target='_blank'; link.rel='noopener noreferrer'; link.textContent='YouTube';
        link.addEventListener('click',function(event){event.stopPropagation();});
        actions.appendChild(link);
      }
      article.append(main,actions); list.appendChild(article);
    });
  }
  ...
  el('shuffle').addEventListener('click',function(){
    var active=this.getAttribute('aria-pressed')!=='true';this.setAttribute('aria-pressed',String(active));window.V2Player.setShuffle(active);
  });
  el('repeat').addEventListener('click',function(){
    var active=this.getAttribute('aria-pressed')!=='true';this.setAttribute('aria-pressed',String(active));window.V2Player.setRepeat(active);
  });
