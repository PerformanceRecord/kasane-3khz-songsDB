...
- `aws s3 cp` で `public-data/songs.json`・`public-data/gags.json`・`public-data/meta.json`・`public-data/archive.json` を個別アップロード
- 各ファイルのアップロードをまとめて判定し、失敗時は最大3回までリトライ
...
5. `public-data/songs.json` / `gags.json` / `meta.json` / `archive.json` が更新されていれば OK
...
加えて、ブラウザまたは `curl` で `https://<公開URL>/public-data/archive.json` が取得できることを確認してください。
