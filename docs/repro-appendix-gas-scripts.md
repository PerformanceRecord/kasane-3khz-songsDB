# 再現用付録: GAS / Spreadsheet スクリプト完全貼り付け版

> この付録は、CODEX から直接操作できない Google Apps Script / Spreadsheet 補助コードを、そのまま貼り付けるための資料です。

## 0. 貼り付け前に必ず置換する値

- `SHEET_ID`（あなたのスプレッドシートID）
- Webアプリの deploy URL / deploy ID（運用環境で再取得）
- CORS の `allowed origins`（あなたのフロント配信ドメイン）

---

## 1. `google-apps-script-reference/code.gs`
...
## 2. `google-apps-script-reference/merge-songs-gags-archive.gs`
...
## 3. `sheet_scripts/performance_record.gs`
